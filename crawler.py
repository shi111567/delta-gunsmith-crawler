...
BUDGET_LOADOUT_TEMPLATES = [
    {
        ...
        "features": "极致性价比，专为跑刀设计，投入极低，捡到就是赚到。",
        "hotScore": 95
    },
    {
        "name": "CAR-15·步枪跑刀",
        ...
        "features": "步枪跑刀，中距离作战能力更强，不虚一般AI。",
        "hotScore": 88
    },
    {
        "name": "M870·卡战备神器",
    ...
    },
    {
        ...
        "map": ["大坝", "航天基地", "巴克什"],
        "weapons": [
            {"name": "MP5冲锋枪", "price": 11.5, "desc": "20米内腰射精准", "code": "MP5冲锋枪-烽火地带-6ICCO940FCDV4UPACJK76"},
        ...
        "features": "经典平民猛攻组合，主副武器搭配，适应多种交战距离。",
        "hotScore": 93
    },
    {
        "name": "MCX LT·零后坐力激光枪",
        ...
        "features": "30万打造T0级体验，指哪打哪，平民战神。",
        "hotScore": 90
    },
    {
        "name": "腾龙·18万S级强度",
        "scenario": "通用",
        "map": ["大坝", "长弓溪谷", "航天基地", "巴克什"],
        ...
        "features": "版本答案，极低成本获得S级强度，无后座体验。",
        "hotScore": 98
    },
]

def update_prices_from_web():
    """尝试从网页抓取最新价格（示例函数，可自行扩展）"""
    # 这里可以写 requests + BeautifulSoup 逻辑，抓取后更新 ARMOR_PRICE_BASE 等字典
    # 为简化演示，直接返回基准价
    print("价格数据已使用 S9 赛季基准价")
    return ARMOR_PRICE_BASE, HELMET_PRICE_BASE, BACKPACK_PRICE_BASE

def generate_budget_loadouts_json():
    """生成最终的起装方案 JSON"""
    # 更新价格（如果爬虫能抓到新价格，在这里替换）
    armor_prices, helmet_prices, backpack_prices = update_prices_from_web()
    
    # 计算总价并更新模板中的价格
    final_loadouts = []
    for tpl in BUDGET_LOADOUT_TEMPLATES:
        total = 0
        # 更新武器价格（如果爬虫能抓到新价格，可在此替换）
        for w in tpl["weapons"]:
            # 可在此根据爬虫数据修正价格
            total += w["price"]
        for a in tpl["armors"]:
            if a["name"] in armor_prices:
                a["price"] = armor_prices[a["name"]]
            total += a["price"]
        for h in tpl["helmets"]:
            if h["name"] in helmet_prices:
                h["price"] = helmet_prices[h["name"]]
            total += h["price"]
        for b in tpl["backpacks"]:
            if b["name"] in backpack_prices:
                b["price"] = backpack_prices[b["name"]]
            total += b["price"]
