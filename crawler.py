...
class QuizGenerator:
    ...
    def add_map_questions(self):
        ...
        for m in maps:
            ...
            options = random.sample([x for x in maps if x != m], 3) + [m]
    ...
    def add_operator_questions(self):
        ...
        for op in operators:
            if op.get("nickname"):
                ...
                options = [o["name"] for o in random.sample(operators, 3)] + [op["name"]]
                random.shuffle(options)
            ...
            options = [o["skill"] for o in random.sample(operators, 3)] + [op["skill"]]
            random.shuffle(options)
    ...
    def add_tactic_questions(self):
        ...
        for t in tactics:
            ...
            options = [x["name"] for x in random.sample(tactics, 3)] + [t["name"]]
            random.shuffle(options)
